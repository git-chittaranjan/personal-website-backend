...
namespace my_api_app.Repositories.Auth.Implementations
{
    public class UserRepository : IUserRepository
    {
        ...
        {
            const string sql = "SELECT UserID, Name, Email, Gender, IsEmailVerified, IsActive, CreatedAt FROM Users WHERE UserID = @UserId;";
        ...
        public async Task<(IEnumerable<UserDetails> Items, int TotalCount)> GetAllUsersAsync(int pageNumber, int pageSize, CancellationToken cancellationToken)
        {
            ...
            const string dataQuery = @"SELECT UserID, Name, Email, Gender, IsEmailVerified, IsActive, CreatedAt FROM Users 
        ...
        {
            const string sql = @"UPDATE Users SET Name = @Name, Gender = @Gender WHERE UserID = @UserID;
                SELECT UserID, Name, Gender, Email, IsEmailVerified, IsActive, CreatedAt FROM Users WITH (NOLOCK)  WHERE UserID = @UserID;";
        ...
        public async Task<UserDetails?> PatchUserAsync(Guid id, PatchUserRequestDto request, CancellationToken cancellationToken)
        {
